...
int main(void)
{
	...
	while(1)
	{
		...
		case SW0_PUSH : 	//SW0
			...
			SerialSendString("AT");
			SerialSendChar(0x0D); // USART1을 통해 PC로 “HELLO! ” 문자열을 전송
			LCD_DisplayText(4,0,"                                    "); 
		...
		case SW3_PUSH : 	//SW3
			...
			SerialSendString("AT+BTNAME=THU04");
			SerialSendChar(0x0D); // USART1을 통해 PC로 “HELLO! ” 문자열을 전송
			LCD_DisplayText(4,0,"                                    "); 
			LCD_DisplayText(4,0,"AT+BTNAME=THU04"); 
			LCD_DisplayText(6,0,"                               ");
			no= 0;
 		break;
		case SW4_PUSH : 	//SW4
