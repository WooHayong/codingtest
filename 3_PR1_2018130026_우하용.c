//////////////////////////////////////////////////////////////////////////
// 3개의 기능(화면)을 갖는 smart watch를 제작한다.
// 각 기능(화면)은 스위치(SW7)를 입력하면 전환이 되도록 한다.
// 초기 화면은 ‘Alarm’, 두번째 화면은 ‘Calculator‘, 세번째 화면은 ‘Thermostat’ 이다.
//세번째 화면에서 SW7을 입력하면 초기화면으로 전환된다.
// 각 화면에는 동일한 시계(시간)가 오른쪽 상단에 항상 표시되도록 한다.
...
void ADC_IRQHandler(void)
{
  if(ModeFlag==2) // Thermostat 모드일때
  {
    ...
    if (ADC2->SR && ADC_SR_EOC == ADC_SR_EOC){ // ADC3 EOC int
      ...
      if( (int)Temperature<=-100 ) // -10도 ~ -10도 일때
      {        
        LCD_DisplayText(1,2,"   ");        
  ...
  if(ModeFlag==1) // 계산기 모드일때
  {
    ...
    if ( (USART1->SR & USART_SR_RXNE) )      // USART_SR_RXNE=(1<<5)
    {
      ...
      case '1' :   // 1 받으면
        ...
        if(Next==0) Operand1= 1;
        else if(Next==2) Operand2=1;
        Next=2;
        break;    
      case '2' :  // 2 받으면
        BEEP();
        LCD_DisplayText(1,2+Next,"2");  
        if(Next==0) Operand1= 2;
        else if(Next==2) Operand2=2;
        Next=2;
        break;    
      case '3' : // 3 받으면
        BEEP();
        LCD_DisplayText(1,2+Next,"3");
        if(Next==0) Operand1= 3;
        else if(Next==2) Operand2=3;
        Next=2;
        break;  
      case '4' :  //  4 받으면
        BEEP();
        LCD_DisplayText(1,2+Next,"4");
        if(Next==0) Operand1= 4;
        else if(Next==2) Operand2=4;
        Next=2;
        break;    
      case '=' :   //  계산 후 모든 변수 초기화
...
void TIMER3_Init(void)
{
  ...
  GPIOB->MODER |= (1<<2*4);	// PB4 Output Alternate function mode				
  GPIOB->OSPEEDR |= (1<<2*4); // PB4 Output speed (100MHz High speed)
  GPIOB->OTYPER &= ~(1<<4); // PB4 Output type push-pull (reset state)
...
void DisplayTitle(void)
{
  ...
  if(ModeFlag==0)  // 알람시계 모드
  {
	...
	TIM4->CR1 &= ~(1<<0);
	GPIOG->ODR &= ~0x00FF;
    LCD_DisplayText(0,0,"1.ALARM(WHY)");  // Title
